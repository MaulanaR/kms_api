...
// RESTAPIHandler provides a convenient interface for KategoriBuku REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the KategoriBuku REST API handler.
...
// GetByID is the REST API handler for `GET /api/kategori_buku/{id}`.
...
// Get is the REST API handler for `GET /api/kategori_buku`.
...
// Create is the REST API handler for `POST /api/kategori_buku`.
...
// UpdateByID is the REST API handler for `PUT /api/kategori_buku/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/kategori_buku/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/kategori_buku/{id}`.
