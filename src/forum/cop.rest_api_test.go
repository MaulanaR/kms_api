...
func prepareTest(tb testing.TB) {
	...
	app.Server().AddRoute("/cop/:id", "GET", REST().GetByID, nil)
	app.Server().AddRoute("/cop/:id", "PUT", REST().UpdateByID, nil)
	app.Server().AddRoute("/cop/:id", "PATCH", REST().PartiallyUpdateByID, nil)
...
var tests = []struct {
	...
	{
		description:  "Get empty list of Cop",
		method:       "GET",
		path:         "/cop",
	...
	{
		description:  "Create Cop with minimum payload",
		method:       "POST",
		path:         "/cop",
		token:        app.TestFullAccessToken,
...
// TestCopREST tests the REST API of Cop data with specified scenario.
func TestCopREST(t *testing.T) {
	prepareTest(t)
...
// BenchmarkCopREST tests the REST API of Cop data with specified scenario.
func BenchmarkCopREST(b *testing.B) {
	b.ReportAllocs()
	prepareTest(b)
	for i := 0; i < b.N; i++ {
		for _, test := range tests {
