...
func (u UseCaseHandler) GetByID(id string) (AdvisKategori, error) {
	res := AdvisKategori{}

	err := u.Ctx.ValidatePermission("advis_kategori.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("advis_kategori.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &AdvisKategori{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &AdvisKategori{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("advis_kategori.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(AdvisKategori{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("advis_kategori.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("advis_kategori.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old AdvisKategori) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
