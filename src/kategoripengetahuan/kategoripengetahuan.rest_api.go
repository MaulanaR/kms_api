...
// RESTAPIHandler provides a convenient interface for KategoriPengetahuan REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the KategoriPengetahuan REST API handler.
...
// GetByID is the REST API handler for `GET /api/kategori_pengetahuan/{id}`.
...
// Get is the REST API handler for `GET /api/kategori_pengetahuan`.
...
// Create is the REST API handler for `POST /api/kategori_pengetahuan`.
...
// UpdateByID is the REST API handler for `PUT /api/kategori_pengetahuan/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/kategori_pengetahuan/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/kategori_pengetahuan/{id}`.
func (r *RESTAPIHandler) DeleteByID(c *fiber.Ctx) error {
	...
	res := map[string]any{
		...
		"message": r.UseCase.Ctx.Trans("deleted", map[string]string{
			"kategori_pengetahuan": p.EndPoint(),
			"id":                   c.Params("id"),
		}),
	}
	return c.JSON(res)
