...
// RESTAPIHandler provides a convenient interface for Notifikasi REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Notifikasi REST API handler.
...
// GetByID is the REST API handler for `GET /api/notifikasi/{id}`.
...
// Get is the REST API handler for `GET /api/notifikasi`.
...
// Create is the REST API handler for `POST /api/notifikasi`.
...
// UpdateByID is the REST API handler for `PUT /api/notifikasi/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/notifikasi/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/notifikasi/{id}`.
