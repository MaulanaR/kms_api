...
// RESTAPIHandler provides a convenient interface for Hadiah REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Hadiah REST API handler.
...
// GetByID is the REST API handler for `GET /api/hadiah/{id}`.
...
// Get is the REST API handler for `GET /api/hadiah`.
...
// Create is the REST API handler for `POST /api/hadiah`.
...
// UpdateByID is the REST API handler for `PUT /api/hadiah/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/hadiah/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/hadiah/{id}`.
...
func (r *RESTAPIHandler) GetRanking(c *fiber.Ctx) error {
	err := r.injectDeps(c)
	if err != nil {
		return app.Error().Handler(c, err)
	}
	res, err := r.UseCase.GetRanking()
	if err != nil {
		return app.Error().Handler(c, err)
	}
	res.SetLink(c)
