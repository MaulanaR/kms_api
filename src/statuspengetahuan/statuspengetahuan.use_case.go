...
// UseCaseHandler provides a convenient interface for StatusPengetahuan use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the StatusPengetahuan data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (StatusPengetahuan, error) {
	res := StatusPengetahuan{}

	// check permission
	err := u.Ctx.ValidatePermission("status_pengetahuan.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of StatusPengetahuan data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("status_pengetahuan.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &StatusPengetahuan{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &StatusPengetahuan{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data StatusPengetahuan with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("status_pengetahuan.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(StatusPengetahuan{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the StatusPengetahuan data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("status_pengetahuan.edit")
	if err != nil {
		return err
	}
	...
	err = tx.Model(&p).Where("id_status_pengetahuan = ?", old.ID).Updates(p).Error
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// PartiallyUpdateByID updates the StatusPengetahuan data for the specified ID with specified parameters.
...
// DeleteByID deletes the StatusPengetahuan data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("status_pengetahuan.delete")
	if err != nil {
		return err
	}
	...
	// update data on the db
	err = tx.Model(&p).Where("id_status_pengetahuan = ?", old.ID).Update("deleted_at", time.Now().UTC()).Error
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// setDefaultValue set default value of undefined field when create or update StatusPengetahuan data.
func (u *UseCaseHandler) setDefaultValue(old StatusPengetahuan) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
