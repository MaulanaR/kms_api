...
// RESTAPIHandler provides a convenient interface for Tag REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Tag REST API handler.
...
// GetByID is the REST API handler for `GET /api/tag/{id}`.
...
// Get is the REST API handler for `GET /api/tag`.
...
// Create is the REST API handler for `POST /api/tag`.
...
// UpdateByID is the REST API handler for `PUT /api/tag/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/tag/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/tag/{id}`.
