...
func (u UseCaseHandler) GetByID(id string) (Pertanyaan, error) {
	res := Pertanyaan{}

	err := u.Ctx.ValidatePermission("pertanyaan.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("pertanyaan.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Pertanyaan{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &Pertanyaan{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("pertanyaan.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Pertanyaan{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("pertanyaan.edit")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old Pertanyaan) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
...
func (u UseCaseHandler) PostJawaban(id string, p *PostJawaban) error {
	err := u.Ctx.ValidateParam(p)
	if err != nil {
		return err
	}
	...
	idPertanyaan, err := strconv.Atoi(id)
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}

	p.IDPertanyaan.Set(int64(idPertanyaan))
