...
// RESTAPIHandler provides a convenient interface for Pertanyaan REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Pertanyaan REST API handler.
...
// GetByID is the REST API handler for `GET /api/pertanyaan/{id}`.
...
// Get is the REST API handler for `GET /api/pertanyaan`.
...
// Create is the REST API handler for `POST /api/pertanyaan`.
...
// UpdateByID is the REST API handler for `PUT /api/pertanyaan/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/pertanyaan/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/pertanyaan/{id}`.
...
// PostJawaban is the REST API handler for `PUT /api/pertanyaan/{id}`.
func (r *RESTAPIHandler) PostJawaban(c *fiber.Ctx) error {
	err := r.injectDeps(c)
	if err != nil {
		return app.Error().Handler(c, err)
	}
	...
	err = r.UseCase.PostJawaban(c.Params("id"), &p)
	if err != nil {
		return app.Error().Handler(c, err)
	}
...
func (r *RESTAPIHandler) DeleteJawabanByID(c *fiber.Ctx) error {
	...
	p := ParamJawabanDelete{}
	p.Ctx = r.UseCase.Ctx
	err = r.UseCase.DeleteJawabanByID(c.Params("id"), &p)
	if err != nil {
		return app.Error().Handler(c, err)
	}
	res := map[string]any{
