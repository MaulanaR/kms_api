...
// RESTAPIHandler provides a convenient interface for Cop REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Cop REST API handler.
...
// GetByID is the REST API handler for `GET /api/cop/{id}`.
...
// Get is the REST API handler for `GET /api/cop`.
...
// UpdateByID is the REST API handler for `PUT /api/cop/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/cop/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/cop/{id}`.
