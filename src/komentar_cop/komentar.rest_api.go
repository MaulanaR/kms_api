...
// RESTAPIHandler provides a convenient interface for Komentar REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Komentar REST API handler.
...
// GetByID is the REST API handler for `GET /api/komentar/{id}`.
...
// Get is the REST API handler for `GET /api/komentar`.
...
// Create is the REST API handler for `POST /api/komentar`.
...
// UpdateByID is the REST API handler for `PUT /api/komentar/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/komentar/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/komentar/{id}`.
