...
// RESTAPIHandler provides a convenient interface for Orang REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Orang REST API handler.
...
// GetByID is the REST API handler for `GET /api/orang/{id}`.
...
// Get is the REST API handler for `GET /api/orang`.
...
// Create is the REST API handler for `POST /api/orang`.
...
// UpdateByID is the REST API handler for `PUT /api/orang/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/orang/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/orang/{id}`.
