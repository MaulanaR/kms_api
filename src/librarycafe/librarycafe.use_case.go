...
func (u UseCaseHandler) GetByID(id string) (LibraryCafe, error) {
	...
	//update count view
	tx.Exec("UPDATE t_library_cafe SET count_view = count_view + 1 WHERE id = ?", id)
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	for k, d := range sData {
		...
		var isDisliked bool
		//get is liked & is disliked
		tx.Raw("SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM t_like WHERE id_library_cafe = ? and id_user = ?", d.ID, u.Ctx.User.ID).Scan(&isLiked)
		tx.Raw("SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM t_dislike WHERE id_library_cafe = ? and id_user = ?", d.ID, u.Ctx.User.ID).Scan(&isDisliked)
