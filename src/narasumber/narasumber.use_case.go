...
func (u UseCaseHandler) GetByID(id string) (Narasumber, error) {
	res := Narasumber{}

	err := u.Ctx.ValidatePermission("narasumber.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("narasumber.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Narasumber{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &Narasumber{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("narasumber.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Narasumber{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("narasumber.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("narasumber.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old Narasumber) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
