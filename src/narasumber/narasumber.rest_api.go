...
// RESTAPIHandler provides a convenient interface for Narasumber REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Narasumber REST API handler.
...
// GetByID is the REST API handler for `GET /api/narasumber/{id}`.
...
// Get is the REST API handler for `GET /api/narasumber`.
...
// Create is the REST API handler for `POST /api/narasumber`.
...
// UpdateByID is the REST API handler for `PUT /api/narasumber/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/narasumber/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/narasumber/{id}`.
