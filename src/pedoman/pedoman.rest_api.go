...
// RESTAPIHandler provides a convenient interface for Pedoman REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Pedoman REST API handler.
...
// GetByID is the REST API handler for `GET /api/pedoman/{id}`.
...
// Get is the REST API handler for `GET /api/pedoman`.
...
// Create is the REST API handler for `POST /api/pedoman`.
...
// UpdateByID is the REST API handler for `PUT /api/pedoman/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/pedoman/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/pedoman/{id}`.
