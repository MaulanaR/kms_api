...
func (u UseCaseHandler) GetByID(id string) (Penerbit, error) {
	res := Penerbit{}

	err := u.Ctx.ValidatePermission("penerbit.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("penerbit.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Penerbit{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &Penerbit{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("penerbit.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Penerbit{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("penerbit.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("penerbit.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old Penerbit) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
