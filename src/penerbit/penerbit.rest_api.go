...
// RESTAPIHandler provides a convenient interface for Penerbit REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Penerbit REST API handler.
...
// GetByID is the REST API handler for `GET /api/penerbit/{id}`.
...
// Get is the REST API handler for `GET /api/penerbit`.
...
// Create is the REST API handler for `POST /api/penerbit`.
...
// UpdateByID is the REST API handler for `PUT /api/penerbit/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/penerbit/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/penerbit/{id}`.
