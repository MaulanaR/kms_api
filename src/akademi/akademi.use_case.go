...
// UseCaseHandler provides a convenient interface for Akademi use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Akademi data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Akademi, error) {
	res := Akademi{}

	// check permission
	err := u.Ctx.ValidatePermission("akademi.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Akademi data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("akademi.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Akademi{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Akademi{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Akademi with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("akademi.create")
	if err != nil {
		return err
	}
...
// UpdateByID updates the Akademi data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("akademi.edit")
	if err != nil {
		return err
	}
	...
	err = tx.Model(&p).Where("id_akademi = ?", old.ID).Updates(p).Error
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// PartiallyUpdateByID updates the Akademi data for the specified ID with specified parameters.
...
// DeleteByID deletes the Akademi data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("akademi.delete")
	if err != nil {
		return err
	}
	...
	// update data on the db
	err = tx.Model(&p).Where("id_akademi = ?", old.ID).Update("deleted_at", time.Now().UTC()).Error
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// setDefaultValue set default value of undefined field when create or update Akademi data.
func (u *UseCaseHandler) setDefaultValue(old Akademi) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
