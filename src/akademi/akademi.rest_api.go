...
// RESTAPIHandler provides a convenient interface for Akademi REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Akademi REST API handler.
...
// GetByID is the REST API handler for `GET /api/akademi/{id}`.
...
// Get is the REST API handler for `GET /api/akademi`.
...
// Create is the REST API handler for `POST /api/akademi`.
...
// UpdateByID is the REST API handler for `PUT /api/akademi/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/akademi/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/akademi/{id}`.
