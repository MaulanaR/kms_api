...
// RESTAPIHandler provides a convenient interface for Pengetahuan REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Pengetahuan REST API handler.
...
// GetByID is the REST API handler for `GET /api/pengetahuan/{id}`.
...
// Get is the REST API handler for `GET /api/pengetahuan`.
...
// Create is the REST API handler for `POST /api/pengetahuan`.
...
// UpdateByID is the REST API handler for `PUT /api/pengetahuan/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/pengetahuan/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/pengetahuan/{id}`.
...
func (r *RESTAPIHandler) GetMixSlider(c *fiber.Ctx) error {
	err := r.injectDeps(c)
	if err != nil {
		return app.Error().Handler(c, err)
	}
	res, err := r.UseCase.GetMixSlider()
	if err != nil {
		return app.Error().Handler(c, err)
	}
	if r.UseCase.IsFlat() {
