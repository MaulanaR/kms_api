...
func (u UseCaseHandler) GetByID(id string) (DokumenMap, error) {
	res := DokumenMap{}

	err := u.Ctx.ValidatePermission("dokumen_map.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("dokumen_map.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &DokumenMap{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &DokumenMap{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("dokumen_map.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(DokumenMap{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("dokumen_map.edit")
	if err != nil {
		return err
	}
	...
	err = tx.Model(&p).Where("id_pengetahuan_dokumen = ?", old.ID).Updates(p).Error
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("dokumen_map.delete")
	if err != nil {
		return err
	}
	...
	err = tx.Model(&p).Where("id_pengetahuan_dokumen = ?", old.ID).Update("deleted_at", time.Now().UTC()).Error
	if err != nil {
		return app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u *UseCaseHandler) setDefaultValue(old DokumenMap) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
