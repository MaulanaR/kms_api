...
// RESTAPIHandler provides a convenient interface for Pulau REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Pulau REST API handler.
...
// GetByID is the REST API handler for `GET /api/pulau/{id}`.
...
// Get is the REST API handler for `GET /api/pulau`.
...
// Create is the REST API handler for `POST /api/pulau`.
...
// UpdateByID is the REST API handler for `PUT /api/pulau/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/pulau/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/pulau/{id}`.
