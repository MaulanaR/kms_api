...
func (u UseCaseHandler) GetByID(id string) (Pulau, error) {
	res := Pulau{}

	err := u.Ctx.ValidatePermission("pulau.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	//RUN QUERY UNTUK SET ID AUTO INCREMENT
	tx, err := u.Ctx.DB()
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}

	err = tx.Exec("ALTER TABLE " + Pulau{}.TableName() + " MODIFY COLUMN id INT AUTO_INCREMENT;").Error
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Pulau{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &Pulau{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("pulau.create")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("pulau.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("pulau.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old Pulau) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
