...
func (u UseCaseHandler) GetByID(id string) (EventMateri, error) {
	res := EventMateri{}

	err := u.Ctx.ValidatePermission("event_materi.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("event_materi.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &EventMateri{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &EventMateri{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	err := u.Ctx.ValidatePermission("event_materi.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(EventMateri{})
	if err != nil {
		return err
	}
	...
	if len(p.Dokumen) > 0 {
		for i, ref := range p.Dokumen {
			...
			p.Dokumen[i].EventMateriID.Set(p.ID.Int64)
		}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("event_materi.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("event_materi.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old EventMateri) error {
	if old.ID.Valid {
		u.ID = old.ID
	}

	//validasi event
	_, err := u.GetEventByID(strconv.Itoa(int(u.EventID.Int64)))
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) GetEventByID(id string) (Event, error) {
	res := Event{}
	...
	q := url.Values{}
	q.Add(key, id)
	err = app.Query().First(tx, &res, q)
	if err != nil {
		return res, u.Ctx.NotFoundError(err, res.EndPoint(), key, id)
	}
