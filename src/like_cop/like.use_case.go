...
func (u UseCaseHandler) GetByID(id string) (LikeCOP, error) {
	res := LikeCOP{}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &LikeCOP{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &LikeCOP{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u *UseCaseHandler) setDefaultValue(old LikeCOP) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
