...
func (u UseCaseHandler) GetByID(id string) (AdvisSumberData, error) {
	res := AdvisSumberData{}

	err := u.Ctx.ValidatePermission("advis_sumber_data.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("advis_sumber_data.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &AdvisSumberData{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &AdvisSumberData{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("advis_sumber_data.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(AdvisSumberData{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("advis_sumber_data.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("advis_sumber_data.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old AdvisSumberData) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
