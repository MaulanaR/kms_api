...
// RESTAPIHandler provides a convenient interface for Kompetensi REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Kompetensi REST API handler.
...
// GetByID is the REST API handler for `GET /api/kompetensi/{id}`.
...
// Get is the REST API handler for `GET /api/kompetensi`.
...
// Create is the REST API handler for `POST /api/kompetensi`.
...
// UpdateByID is the REST API handler for `PUT /api/kompetensi/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/kompetensi/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/kompetensi/{id}`.
