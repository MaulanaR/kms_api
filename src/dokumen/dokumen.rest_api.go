...
// RESTAPIHandler provides a convenient interface for Dokumen REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Dokumen REST API handler.
...
// GetByID is the REST API handler for `GET /api/dokumen/{id}`.
...
// Get is the REST API handler for `GET /api/dokumen`.
...
// Create is the REST API handler for `POST /api/dokumen`.
...
// UpdateByID is the REST API handler for `PUT /api/dokumen/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/dokumen/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/dokumen/{id}`.
