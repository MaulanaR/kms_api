...
// RESTAPIHandler provides a convenient interface for SubjenisPengetahuan REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the SubjenisPengetahuan REST API handler.
...
// GetByID is the REST API handler for `GET /api/subjenis_pengetahuan/{id}`.
...
// Get is the REST API handler for `GET /api/subjenis_pengetahuan`.
...
// Create is the REST API handler for `POST /api/subjenis_pengetahuan`.
...
// UpdateByID is the REST API handler for `PUT /api/subjenis_pengetahuan/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/subjenis_pengetahuan/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/subjenis_pengetahuan/{id}`.
func (r *RESTAPIHandler) DeleteByID(c *fiber.Ctx) error {
	...
	res := map[string]any{
		...
		"message": r.UseCase.Ctx.Trans("deleted", map[string]string{
			"subjenis_pengetahuan": p.EndPoint(),
			"id":                   c.Params("id"),
		}),
	}
	return c.JSON(res)
