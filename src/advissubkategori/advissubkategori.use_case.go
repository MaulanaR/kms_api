...
func (u UseCaseHandler) GetByID(id string) (AdvisSubKategori, error) {
	res := AdvisSubKategori{}

	err := u.Ctx.ValidatePermission("advis_sub_kategori.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("advis_sub_kategori.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &AdvisSubKategori{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &AdvisSubKategori{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("advis_sub_kategori.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(AdvisSubKategori{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("advis_sub_kategori.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("advis_sub_kategori.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old AdvisSubKategori) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
