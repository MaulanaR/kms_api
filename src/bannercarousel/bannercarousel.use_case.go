...
func (u UseCaseHandler) GetByID(id string) (BannerCarousel, error) {
	res := BannerCarousel{}

	err := u.Ctx.ValidatePermission("banner_carousel.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("banner_carousel.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &BannerCarousel{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &BannerCarousel{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("banner_carousel.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(BannerCarousel{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("banner_carousel.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("banner_carousel.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old BannerCarousel) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
