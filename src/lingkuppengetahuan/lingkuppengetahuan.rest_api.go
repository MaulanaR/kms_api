...
// RESTAPIHandler provides a convenient interface for LingkupPengetahuan REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the LingkupPengetahuan REST API handler.
...
// GetByID is the REST API handler for `GET /api/lingkup_pengetahuan/{id}`.
...
// Get is the REST API handler for `GET /api/lingkup_pengetahuan`.
...
// Create is the REST API handler for `POST /api/lingkup_pengetahuan`.
...
// UpdateByID is the REST API handler for `PUT /api/lingkup_pengetahuan/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/lingkup_pengetahuan/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/lingkup_pengetahuan/{id}`.
func (r *RESTAPIHandler) DeleteByID(c *fiber.Ctx) error {
	...
	res := map[string]any{
		...
		"message": r.UseCase.Ctx.Trans("deleted", map[string]string{
			"lingkup_pengetahuan": p.EndPoint(),
			"id":                  c.Params("id"),
		}),
	}
	return c.JSON(res)
