...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	for k, d := range data {
		...
		var isDisliked bool
		//get is liked & is disliked
		tx.Raw("SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM t_like WHERE id_leader_talk = ? and id_user = ?", d["id"].(int32), u.Ctx.User.ID).Scan(&isLiked)
		tx.Raw("SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM t_dislike WHERE id_leader_talk = ? and id_user = ?", d["id"].(int32), u.Ctx.User.ID).Scan(&isDisliked)
