...
func (u UseCaseHandler) GetByID(id string) (KelompokDokumen, error) {
	res := KelompokDokumen{}

	err := u.Ctx.ValidatePermission("kelompok_dokumen.detail")
	if err != nil {
		return res, err
	}
...
func (u UseCaseHandler) Get() (app.ListModel, error) {
	...
	err := u.Ctx.ValidatePermission("kelompok_dokumen.list")
	if err != nil {
		return res, err
	}
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &KelompokDokumen{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	data, err := app.Query().Find(tx, &KelompokDokumen{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
func (u UseCaseHandler) Create(p *ParamCreate) error {
	...
	err := u.Ctx.ValidatePermission("kelompok_dokumen.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(KelompokDokumen{})
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {
	...
	err := u.Ctx.ValidatePermission("kelompok_dokumen.edit")
	if err != nil {
		return err
	}
...
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {
	...
	err := u.Ctx.ValidatePermission("kelompok_dokumen.delete")
	if err != nil {
		return err
	}
...
func (u *UseCaseHandler) setDefaultValue(old KelompokDokumen) error {
	if old.ID.Valid {
		u.ID = old.ID
	}
